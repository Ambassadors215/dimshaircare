...
export default async function handler(req, res) {
  ...
  const closedBanner = storeOpen
    ? ""
    : `<div style="background:#7f1d1d;color:#fff;padding:14px 16px;border-radius:12px;margin-bottom:16px;font-weight:600;text-align:center">This store is currently <strong>closed</strong> for orders. Please check back later.</div>`;
  const reviewsArr = Array.isArray(listing.reviews) ? listing.reviews : [];
  const reviewsBlock =
    reviewsArr.length > 0
      ? reviewsArr
          .map((r) => {
            const rt = Math.min(5, Math.max(1, Math.round(Number(r.rating) || 5)));
            const stars = "★".repeat(rt) + "☆".repeat(5 - rt);
            const reply =
              r.reply && String(r.reply).trim()
                ? `<p class="muted" style="margin-top:10px;font-size:0.9rem"><strong>Store reply:</strong> ${esc(String(r.reply))}</p>`
                : "";
            return `<div style="border-bottom:1px solid rgba(139,58,58,.2);padding:12px 0">
  <p style="margin:0 0 6px;color:var(--gold);letter-spacing:1px">${stars}</p>
  <p style="margin:0;font-size:0.95rem">${esc(String(r.text || ""))}</p>
  <p class="muted" style="margin:8px 0 0;font-size:0.85rem">${esc(String(r.customerFirstName || "Customer"))} · ${esc(String(r.productName || ""))} · ${esc(String(r.date || "").slice(0, 10))}</p>
  ${reply}
</div>`;
          })
          .join("")
    ...
    priceList.length > 0
      ? `<div class="grid" id="product-grid">${priceList
          .map((r, i) => {
            ...
            const sp = storeProducts[i];
            const oos = sp && sp.inStock === false;
            const feat = sp && sp.featured;
            return `<article class="pcard" data-idx="${i}">
  ${feat ? `<span class="tag" style="display:inline-block;margin-bottom:8px;background:var(--gold);color:var(--brown)">Featured</span>` : ""}
  <a class="plink" href="${pu}"><span class="pname">${esc(r.item)}</span></a>
  <p class="pprice">${esc(r.price)}${oos ? " · <span style=color:#b91c1c>Out of stock</span>" : ""}</p>
  <div class="prow">
    <button type="button" class="btn-add" data-add="${i}" ${oos || !storeOpen ? "disabled style=opacity:.5" : ""}>${oos ? "Unavailable" : "Add to basket"}</button>
...
(function(){
  ...
  document.getElementById("product-grid") && document.getElementById("product-grid").addEventListener("click", function(ev){
    ...
    if (t && t.getAttribute("data-add") != null) {
      ...
      var row = STORE.priceList[idx|0];
      if (row && row.inStock === false) return;
      var c = getCart();
      c[idx] = (c[idx]|0) + 1;
      setCart(c);
      render();
  ...
  document.getElementById("open-checkout").addEventListener("click", function(){
    if (!STORE.storeOpen) { alert("This store is closed for orders right now."); return; }
    var c = getCart();
    var lines = [];
    var summary = [];
    for (var k in c) {
      ...
      if (!row || row.inStock === false) continue;
  ...
  document.getElementById("checkout-form").addEventListener("submit", async function(ev){
    ...
    for (var k in c) {
      ...
      if (qty < 1) continue;
      var ix = k|0;
      var pr = STORE.priceList[ix];
      if (!pr || pr.inStock === false) continue;
      cartLines.push({ idx: ix, qty: qty });
    }
    if (!STORE.storeOpen) { err.textContent = "This store is closed for orders."; return; }
