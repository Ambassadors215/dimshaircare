...
export default async function handler(req, res) {
  ...
  if (!listing || !row || !String(row.item || "").trim()) {
    ...
    res.end(
      `<!DOCTYPE html><html><head><title>Not found</title></head><body><p>Product not found.</p><p><a href="/store/${encodeURIComponent(storeId)}">Back to store</a></p></body></html>`
    );
    return;
  }
...
<p><a href="/store/${encodeURIComponent(storeId)}">← ${esc(storeName)}</a> · <a href="/stores">All stores</a></p>
...
<p><a href="/store/${encodeURIComponent(storeId)}" style="font-weight:700;color:#8B3A3A">View full store</a></p>
</body>
</html>`;
