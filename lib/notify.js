...
export async function notifyNegotiationUpdate(neg, action, recipientEmail) {
  ...
  const dashUrl = recipientEmail === neg.customerEmail
    ? `/user/?email=${encodeURIComponent(recipientEmail)}`
    : `/provider/?email=${encodeURIComponent(recipientEmail)}`;
...
<p><a href="${esc(process.env.SITE_URL || "https://clips-service.vercel.app")}${dashUrl}">View in your dashboard</a></p>
<p>— Clip Services</p>`;
  await parallel(
    isEmailConfigured()
      ...
      : Promise.resolve(),
    notifyPushRole("admin", {
      title: actionLabels[action] || "Negotiation update",
      body: `${neg.id} · ${neg.service}`,
      url: "/admin/",
    })
  );
}
